function importJiraTasks() {
  // Load credentials from ENV (env.gs)
  const email = ENV.EMAIL;
  const apiToken = ENV.API_TOKEN;
  const domain = ENV.DOMAIN;
  const projectName = ENV.PROJECT_NAME;
  const jql = `project = ${projectName} AND issuetype = Task ORDER BY created DESC`;
  ...
  const statusOptions = ["To Do", "In Progress","Testing", "Pushed to Staging", "Tested & Ready for Prod", "Released", "On Hold", "Being Designed", "🚀 Released"];

  const response = UrlFetchApp.fetch(url, { headers });
  const issues = JSON.parse(response.getContentText()).issues;

  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName("Jira Tasks");
  if (!sheet) {
    sheet = ss.insertSheet("Jira Tasks");
    sheet.appendRow([
